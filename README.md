...
- 🎤 **浏览器录制**：通过麦克风录制或上传已有音频
- 📝 **文本对齐**：基于 Whisper 强制对齐获取真实 duration
...
- 📦 **一键导出**：导出 ZIP 包（ONNX + config + 参考音频）供 LiveBuddy 导入
- 🚀 **GPT-SoVITS 集成（实验性）**：支持调用 GPT-SoVITS 大模型进行 zero-shot 试听（仅供对比，**不可导出 ONNX**）
